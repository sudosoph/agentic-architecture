...
A meaningful percentage of HITL friction is review latency. The human is approving 80 items in a sitting and the queue is bottlenecked on context.

When the agent runs locally, the artifacts the human needs to approve are pre-fetched, structured, and presented in a tight review interface (a Slack DM, a tiny web app, an inbox plugin). When the agent runs cloud-only with metered API calls, every review item is also a token bill, which means teams ration the context they pre-fetch, which means the human spends more time looking up artifacts manually.

The local-first stack makes the HITL experience materially faster because nothing is rate-limited. The agent can pre-compute as much context per review item as the human will ever need, and the marginal cost is electricity.
