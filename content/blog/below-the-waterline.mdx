...
The work nobody puts on a slide. Auth standardization, logging consistency, error patterns, CI/CD for agent deployment, observability that tells you why something broke (not just that it did). Charity Majors made the canonical case for this with her [Observability 2.0](https://charity.wtf/tag/observability-2-0/) framing: arbitrarily-wide structured events, single source of truth, no more three-pillars (metrics + logs + traces) silos that destroy the relational context. For agentic systems, that exact model is the one that scales.
