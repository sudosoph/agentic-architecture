...
## Why local-first sits underneath all three

The three foundations are not optional. They are also significantly easier to build when meaningful inference is local.

**Developer experience is easier locally.** Logging, tracing, debugging are all simpler when the model runs on the same machine as the platform middleware. Cloud-only setups multiply network hops, multiply failure modes, and force you to reproduce production conditions in a sandbox before debugging.

**Agent identity is simpler locally.** The 4-legged identity problem (user, agent, MCP, API) becomes the 1-legged identity problem (just file permissions and env vars) when everything runs on a single machine. The MCP spec itself acknowledges this: stdio transport explicitly opts out of OAuth.

**Token-factory economics is built on local inference.** The token factory that can route 35% of traffic to local hardware has a structural margin advantage over competitors who cannot. The capital expense is a one-time hardware cost. The variable cost is electricity.

A team that ignores local-first builds the same three foundations as a team that embraces it. The local-first team builds them once, runs them at lower marginal cost, and serves regulated customers cloud-only competitors cannot reach.
