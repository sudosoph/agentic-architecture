...
Borrowed wholesale from cognitive science. The Replit founders, Oracle's AI team, and several speakers at AI Dev SF have all converged on this taxonomy. It works because it actually maps to the storage decisions you have to make.
...
A long-running memory system on metered API rates is expensive. Every retrieval is a token charge. Every summarization is a token charge. At episodic-log scale (hundreds of events per day), the costs add up to a real line item.

On hardware you own, the writes are free. You can re-summarize the entire month's episodic log at the end of every week and not feel it. The agent gets the benefit of compounding memory without the bill that makes memory feel expensive.

The semantic graph (Neo4j) and the episodic log (filesystem + vector index) both run locally on the same Framework 16 that runs the model. No network egress. No third-party data. The "personal" in personal AI agent means the agent's memory is also yours.
