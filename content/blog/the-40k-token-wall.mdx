...
The bigger context window is not just a quality liability. It is a cost liability. Past 40K tokens, every turn is paying for context you cannot fully use, billed at the same rate as productive context.

On hardware you own, the same loop is electricity, and you can afford to be aggressive about scoping. A local agent at 10K tokens beats a cloud agent at 200K tokens on cost, latency, and quality. The model running on a Framework 16 with 96GB unified memory is a Qwen 3.6 27B or a Gemma 4 26B MoE. Smaller than Opus. Sharp inside its window. Cheap enough to leave running through every retrieval loop.

The pattern for sub-frontier tasks: keep the context tight, the loop tighter, the model local. Reach for the cloud only when a single turn genuinely needs the smartest model in the world. Most do not.
