...
## When local-first matters here

Shadow mode generates production-volume traces of agent reasoning. At Bain's 3K emails per day, that is 3K full agent traces. Each trace contains the system prompt, the tool catalog, the reasoning history, the validator outputs, the action plan, the diff to the would-be system state.

On a metered API, paying full Sonnet 4.5 rates to generate shadow traces at 3K per day is a budget item. ~6.5M input + 432K output per session, multiplied by enough developers running shadow tests, is real money.

On hardware you own, shadow mode is electricity. You run the agent in shadow against the same volume the production system sees, for as long as you want, at no marginal cost. You can leave it shadow-running through the night for a month while you tune. The Bain team had four weeks at 3K/day. Most teams never get there because the API bill kills the experiment first.
